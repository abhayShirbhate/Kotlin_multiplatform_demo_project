...
internal data class ActionFinder(
  ...
  private fun List<SwipeAction>.actionAt(offset: Float, totalWidth: Int, userModel: User? = null): SwipeAction? {
    if (isEmpty()) {
      return null
    }
    ...
    for (i in 0 until size) {
      val action = if (userModel?.isFavorite == true) this[0] else this[1]
